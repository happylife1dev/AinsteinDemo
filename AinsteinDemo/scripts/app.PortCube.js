...
$('#portfolioform').submit(function () {
    $('#portcubeframe').prop('src', '/Portfolio/Test/' + $('#txtPortfolioInput').val());
    return false;
});
...
function getRatingStyle(rating) {
    switch (rating) {
        case "A":
            style = "rating rating_text_A";
            break;
        case "B":
            style = "rating rating_text_B";
            break;
        case "C":
            style = "rating rating_text_C";
            break;
        case "D":
            style = "rating rating_text_D";
            break;
        case "E":
            style = "rating rating_text_E";
            break;
        case "F":
            style = "rating rating_text_F";
            break;
        default:
            style = "rating rating_text_NONE";
    }
    return style;
}

function changeRibbonRatingStyle(fieldName, value) {
    var newStyle = "rating_text_" + value;

    const fieldElement = window.parent.parent.document.getElementById(fieldName);

    fieldElement.classList.remove("rating_text_A");
    fieldElement.classList.remove("rating_text_B");
    fieldElement.classList.remove("rating_text_C");
    fieldElement.classList.remove("rating_text_D");
    fieldElement.classList.remove("rating_text_E");
    fieldElement.classList.remove("rating_text_F");

    fieldElement.classList.add(newStyle);
}
...
function checkHighlight() {
    ...
    if ((intersects.length > 0) && (lastEvent != "drag")) { // case if mouse is not currently over an object
        ...
        //determine letter grade for ssrating (color of sphere) 
        switch (true) {
        ...
        //determine letter grade for esg_risk
        switch (true) {
        ...
        //CREATE TOOL TYPE HEADER
        //if navigator platform detector contains any of these strings: iPhone, iPod, or iPad
        if (isiOS()) {
            text.innerHTML = '<div onclick="return false;"><span class="tooltip-header">' + hoverName + '</span><br><span class="tootip-ticker">' + hoverID + '</span></div>';

            //in iOS, include both ID and name if different
            if (hoverName != hoverID) {
                text.innerHTML += '<span onclick="return false;">' + hoverName + ' (' + hoverID + ')</span>';
            }

            //desktop
        } else {
            text.innerHTML = '<div onclick="return false;"><span class="tooltip-header">' + hoverName + '</span><span class="tootip-ticker">' + hoverID + '</span></div>';
        }
                ...
                if (window.parent.parent.document.getElementById("tooltip_container")) {
                    $(document).ready(function () {
                        $.ajax({
                            ...
                            data: {
                                "tickerSymbol": hoverID
                            },
                            type: "GET",
                            contentType: "json",
                            success: function (data) {
                                window.parent.parent.document.querySelector(".ribbon-wrapper").style = "display;block";
                        ...
                        //header
                        //$(window.parent.parent.document.getElementById('vecSym')).html(`<a class="vector-drilldown-anchor" href="/Company/Details/${hoverID}" target="_top">a` + hoverName + "&nbsp;<span class='color'" + theLet + '>&nbsp;(' + hoverID + ')&nbsp;</span></a>');
                        //$('#company-name').html(hoverName);
                        //$('#tooltip-company-name:first').html('<span>12341234</span>');
                        //$('#company-symbol').html(hoverID);
                        //$('#company-cap').html(hoverDispValue + hoverVLabel);
                    });
                    ...
                    //market cap
                    //$(window.parent.parent.document.getElementById('vecMCap')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">b' + hoverVTitle + '=' + hoverDispValue + hoverVLabel + '</p>')

                    //ainstein rating
                    //$(window.parent.parent.document.getElementById('vecRT')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">c' + hoverCTitle + '= ' + theLet + '</p>')

                    //value
                    $(window.parent.parent.document.getElementById('vecPE')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">d' + hoverYTitle + '= ' + hoverYValue + ' X' + '</p>')

                    //earnings growth
                    $(window.parent.parent.document.getElementById('vecEG')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">e' + hoverZTitle + '= ' + '(' + hoverZValue / 100 + ') %' + '</p>')

                    //trading risk
                    $(window.parent.parent.document.getElementById('vecTR')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">f' + hoverXTitle + '= ' + hoverXValue + '</p>')

                    //esg risk
                    $(window.parent.parent.document.getElementById('vecESG')).html('<p class="vector-drilldown-paragraph href="/Company/Details/${hoverID}" target="_top">gESG Risk = ' + theEsgLet + '</p>')
                    */
                    //window.parent.parent.document.getElementById("bubble-popup-wrapper").style.display = "none";
        ...
        if ((lastEvent == "mouseUp") || isiOS()) {

            window.parent.parent.document.getElementById("bubble-popup-wrapper").style.display = "block";
            // company - details
            window.parent.parent.document.getElementById("popup-company-name").innerText = hoverName;
            window.parent.parent.document.getElementById("company-symbol").innerText = hoverID;
            // market cap - details
            window.parent.parent.document.getElementById("company-cap").innerText = 'Market Cap $' + hoverDispValue + hoverVLabel;
            // rating details - details
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_A').innerText = theLet;
            // value details
            window.parent.parent.document.getElementById("value-rating").innerText = hoverYValue + 'X';
            // earnings-rating details
            window.parent.parent.document.getElementById("earnings-rating").innerText = hoverZValue / 100 + '%';
            // trading details -- value is incorrect field 'hoverXValue' so keeping hard coded
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_D').innerText = 'D';
            // esg risk details
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_B').innerText = theEsgLet;
        }
        updateDetails();
...
function displayOnWindowLoad() {

    let targetObject = json.data.sort((a, b) => parseFloat(b.marketcap) - parseFloat(a.marketcap))[0];

    const targetId = targetObject.id;

    switch (true) {
        case (targetObject.overallrating < 1.5):
            theLet = 'A';
            break;
        case (targetObject.overallrating <= 2.5):
            theLet = 'B';
            break;
        case (targetObject.overallrating <= 3.5):
            theLet = 'C';
            break;
        case (targetObject.overallrating <= 4.5):
            theLet = 'D';
            break;
        default:
            theLet = 'F';
    ...
    //determine letter grade for esg_risk
    switch (true) {
        case (targetObject.tradingrisk < 1.5):
            theEsgLet = 'A';
            break;
        case (targetObject.tradingrisk <= 2.5):
            theEsgLet = 'B';
            break;
        case (targetObject.tradingrisk <= 3.5):
            theEsgLet = 'C';
            break;
        case (targetObject.tradingrisk <= 4.5):
            theEsgLet = 'D';
            break;
        default:
            theEsgLet = 'F';
    ...
    //determine value magnification
    switch (true) {
        case (targetObject.marketcap >= 1000000):
            hoverDispValue = addCommas((targetObject.marketcap / 1000000).toFixed(0));
            hoverVLabel = ' Trillion';
            break;
        case (targetObject.marketca >= 1000):
            hoverDispValue = addCommas((targetObject.marketcap / 1000).toFixed(0));
            hoverVLabel = ' Billion';
            break;
        default:
            hoverDispValue = addCommas((targetObject.marketcap / 1).toFixed(0));
            hoverVLabel = ' Million';
            break;
    }

    if (window.parent.parent.document.getElementById("bubble-popup-wrapper")) {
        $(document).ready(function () {
            lastEvent = null;
            window.parent.parent.document.getElementById("bubble-popup-wrapper").style.setProperty("display", "block");
            //console.log(window.parent.parent.document.getElementById("bubble-popup-wrapper"))
            // company - details
            window.parent.parent.document.getElementById("popup-company-name").innerText = targetObject.name;
            window.parent.parent.document.getElementById("company-symbol").innerText = targetObject.id;
            // market cap - details
            window.parent.parent.document.getElementById("company-cap").innerText = 'Market Cap $' + hoverDispValue + hoverVLabel;
            // rating details - details
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_A').innerText = theLet;
            // value details
            window.parent.parent.document.getElementById("value-rating").innerText = targetObject.pemultiple + 'X';
            // earnings-rating details
            window.parent.parent.document.getElementById("earnings-rating").innerText = targetObject.earningsgrowth / 100 + '%';
            // trading details -- value is incorrect field 'hoverXValue' so keeping hard coded
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_D').innerText = 'D';
            // esg risk details
            window.parent.parent.document.querySelector('#ainstein-rating .rating_text_B').innerText = theEsgLet;
